...
ImGuiAl::Fifo::Fifo(void *const buffer, size_t const size)
    : _buffer(buffer), _size(size), _available(size), _first(0), _last(0) {}
...
ImGuiAl::Crt::Crt(void *const buffer, size_t const size)
    : _fifo(buffer, size), _foregroundColor(CGA::White), _metaData(0), _scrollToBottom(false) {}
...
ImGuiAl::Log::Log(void *const buffer, size_t const buffer_size)
    : Crt(buffer, buffer_size), _debugLabel("Debug"), _infoLabel("Info"), _warningLabel("Warning"), _errorLabel("Error"), _cumulativeLabel("Cumulative"), _filterLabel("Filter (inc,-exc)"), _filterHeaderLabel(nullptr), _actions(nullptr), _level(Level::Debug), _cumulative(true)
{
...
ImGuiAl::Terminal::Terminal(void *const buffer,
    ...
    : Crt(buffer, buffer_size), _commandBuffer(static_cast<char *>(cmd_buf)), _cmdBufferSize(cmd_size), _execute(std::move(execute)), _callback(std::move(callback))
{
